...
fn sinf(mut x: f32) -> f32 {
    ...
    let sinf_imp = |x: f32| -> f32 {
        // These magic number were discovered 1400 years ago!
...
impl State {
    ...
    pub fn get_view(&self) -> [(i32, bool); 160] {
        // The player's FOV is split in half by their viewing angle. In order to get the ray's first angle we must add half the FOV to the player's angle to get the edge of the player's FOV.
        let starting_angle = self.player_angle + HALF_FOV;

        let mut walls = [(0, false); 160];

        for (idx, wall) in walls.iter_mut().enumerate() {
            // idx is what number ray we are, wall is a mutable reference to a value in walls.
            let angle = starting_angle - idx as f32 * ANGLE_STEP;

            // Get both the closest horizontal and vertical wall intersections for the angle.
    ...
    fn horizontal_intersection(&self, angle: f32) -> f32 {
        // This tells you if the angle is "facing up" regardless of how big it is.
        let up = fabsf(floorf(angle / PI) % 2.0) != 0.0;

        // first_y and first_x are the grid intersections that the ray intersects with.
        let first_y = if up {
        ...
        // next_x and next_y are mutable values which will keep track of how far away the ray is from the player.
        let mut next_x = first_x;
        let mut next_y = first_y;

        // This is the loop where the ray is extended until it hits the wall. It's not an infinite loop as implied in the explanation, intead it inly goes from 0 to 256.
        // This was chosen because if something goes wrong and the ray never hits a wall (which should never happen) the loop will eventually break and the game will kepp on running.
        for _ in 0..256 {
            // current_x and current_y are where the ray is currently onthe map, while next_x and next_y are relative coordinates, current_x and current_y are absolute points.
            ...
            // If we didn't hit a wall on this extension, add dx and dy to our current position and keep going.
            next_x += dx;
            next_y += dy;
        }
    ...
    /// Returns the nearest wall the ray intersects with a vertical grid line.
    fn vertical_intersection(&self, angle: f32) -> f32 {
        // This tells you if the angle is "facing right" regardless of how bit the angle is.
        ...
        // next_x and next_y are mutable values which will kepp track of how far away the ray is from the player.
        let mut next_x = first_x;
        let mut next_y = first_y;

        // This is the loop where the ray is extended until it hits the wall. It's not an infinite loop as implied in the explanation, instead it only goes from 0 to 256.
        // This was chosen because if something goes wrong and the ray never hits a wall (which should never happen) the loop will eventually quit and the game will keep on running.
        for _ in 0..256 {
            // current_x and current_y are where the ray is currently on the map, while next_x and next_y are relative coordinates, current_x and current_y are absolute points.
